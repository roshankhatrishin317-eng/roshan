...
export class ProviderPoolManager {
    ...
    constructor(providerPools, options = {}) {
        ...
        // 启动后台健康检查任务 (Start background health check task)
    ...
    startBackgroundHealthChecks() {
        ...
        // Run health checks every 'healthCheckInterval' ms
        this.healthCheckTimer = setInterval(() => {
            this.performHealthChecks().catch(err => {
                this._log('error', `Background health check failed: ${err.message}`);
            });
        }, this.healthCheckInterval);
        
        this._log('info', `Background health checks started (interval: ${this.healthCheckInterval}ms)`);
    }
