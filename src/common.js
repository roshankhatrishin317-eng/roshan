...
export async function handleStreamRequest(res, service, model, requestBody, fromProvider, toProvider, PROMPT_LOG_MODE, PROMPT_LOG_FILENAME, providerPoolManager, pooluuid) {
    ...
    // Estimate input tokens (simple estimation: 1 char ~= 0.25 tokens or just 0 for now)
    // Ideally we should use a tokenizer, but for perf we might skip exact count.
    // Or we can try to extract usage if the provider returns it.
    
    // For now, let's roughly track input characters / 4
    const promptText = extractPromptText(requestBody, toProvider);
    const inputTokens = Math.ceil((promptText?.length || 0) / 4);
    metrics.trackTokens(inputTokens, 0);
    ...
    try {
        for await (const nativeChunk of nativeStream) {
            ...
            if (chunkText && !Array.isArray(chunkText)) {
                ...
                // Track output tokens (rough check: length / 4)
                // This is an estimation for streaming as most providers don't send usage in chunks
                const outputTokens = Math.ceil(chunkText.length / 4);
                metrics.trackTokens(0, outputTokens);
            }
...
export async function handleUnaryRequest(res, service, model, requestBody, fromProvider, toProvider, PROMPT_LOG_MODE, PROMPT_LOG_FILENAME, providerPoolManager, pooluuid) {
    try{
        ...
        metrics.trackTokens(inputTokens, outputTokens);
 ...
 */
export async function handleContentGenerationRequest(req, res, service, endpointType, CONFIG, PROMPT_LOG_FILENAME, providerPoolManager, pooluuid, preParsedBody = null) {
    const originalRequestBody = preParsedBody || await getRequestBody(req);
